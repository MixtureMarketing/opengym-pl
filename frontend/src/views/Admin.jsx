...
// Panel operatora dla administratora (klucz dostępu właściciela + flaga admin; sprawdzane
// ponownie po stronie serwera). W tej wersji przetłumaczony — teksty siedzą w pakietach
// językowych razem z resztą interfejsu.
...
const dur = ms => { const m = Math.max(0, Math.floor(ms / 60000)); return m < 60 ? m + t('m') : Math.floor(m / 60) + t('h') + (m % 60) + t('m') }
...
export default function Admin() {
  ...
  // odpytujemy co 15 s, żeby sekcja „trenują teraz" była żywa bez ręcznego odświeżania
    ...
    {liveUsers.length > 0 && <div className="card" style={{ borderColor: 'var(--acc)' }}>
      ...
      {liveUsers.map(u => <div key={u.id} className="row between" style={{ padding: '8px 2px', borderBottom: '1px solid var(--sep)' }} onClick={() => openUser(u.id)}>
        <div><div className="small" style={{ fontWeight: 600 }}>{u.name}</div>
          <div className="dim" style={{ fontSize: '.72rem' }}>{u.live.name} · {t('ex.')} {u.live.exIdx}/{u.live.exTotal} · {t('{0} sets', u.live.setsDone + '/' + u.live.setsTotal)}</div></div>
